...
abstract class Domstor_Detail_Supply extends Domstor_Detail_Common
{
    ...
    protected function getImagesHtml($object)
    {
        ...
        if ($out) {
            $out = '<div class="domstor_images">' . $out . '</div>';
        }
        return $out;
    }
    ...
    public function getAddress()
    {
        ...
        if ($note = $this->getVar('address_note')) {
            $out.= ', ' . $note;
        }
        if ($coop = $this->getVar('cooperative_name')) {
            $out.= ', ' . $coop;
        }
    ...
    public function getLocationBlock()
    {
        ...
        if ($location) {
            ...
            if (isset($a['first_line']) and $a['first_line']) {
                $location.='<p>Первая линия</p>';
            }
            if (isset($a['metro']) and $a['metro']) {
                $location.='<p>' . $a['metro'] . '</p>';
            }
            if (isset($a['available_bus']) and $a['available_bus']) {
                $location.='<p>От остановки ' . $a['available_bus'] . ' мин.</p>';
            }
            if (isset($a['available_metro']) and $a['available_metro']) {
                $location.='<p>От метро ' . $a['available_metro'] . ' мин.</p>';
            }
            if (isset($a['available_bus_to_metro']) and $a['available_bus_to_metro']) {
                $location.='<p>Транспортом до метро ' . $a['available_bus_to_metro'] . ' мин.</p>';
            }
            if (isset($a['map_weblink']) and $a['map_weblink']) {
                $location.='<p><a href="' . $a['map_weblink'] . '" target="_blank">На карте</a></p>';
            }
            if (isset($a['img_map']) and $map = $this->getMapHtml($a['img_map'])) {
                $location.= $map;
            }
            if (isset($a['zone']) and $a['zone']) {
                $location.='<p>Территориальная зона' . $a['zone'] . '</p>';
            }
            $location = '
    ...
    public function getRealizationBlock()
    {
        ...
        if ($a['realization_way_id'] == 1183 or $a['realization_way_id'] == 1184 or $a['realization_way_id'] == 1185) {
            ...
            if ($date) {
                $out.=$this->getElement('Дата проведения:', date('d.m.Y', $date));
            }
            if ($date) {
                $out.=$this->getElement('Время проведения:', date('H:i', $date));
            }
            $out.=$this->getElementIf('Место проведения:', $a['auction_location']);
            $date = strtotime($a['auction_filing_start']);
            if ($date) {
                $out.=$this->getElement('Дата начал подачи заявок:', date('d.m.Y', $date));
            }
            $date = strtotime($a['auction_filing_finish']);
            if ($date) {
                $out.=$this->getElement('Дата окончания подачи заявок:', date('d.m.Y', $date));
            }
        } else {
