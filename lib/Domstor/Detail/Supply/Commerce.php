...
class Domstor_Detail_Supply_Commerce extends Domstor_Detail_Supply
{
    ...
    public function getAnnotation()
    {
        ...
        if ($this->getVar('complex_id')) {
            $annotation.=', В составе имущественного комплекса';
        }
    ...
    public function getDelayBlock()
    {
        $a = &$this->object;
        $out = '';
        if ($this->getVar('delay_sale_dt')) {
            $out.='<p>Отсроченная продажа с '.date('d.m.Y', strtotime($this->getVar('delay_sale_dt'))).'</p>';
        }
        if ($this->getVar('delay_rent_dt')) {
            $out.='<p>Отсроченная аренда с '.date('d.m.Y', strtotime($this->getVar('delay_rent_dt'))).'</p>';
        }
        if ($out) {
    ...
    {
        $a=&$this->object;
        if ($this->getVar('placing_features_id')==-1001) {
            return;
        }
        ...
        $pt=array();
        $inside = $out = '';
        if ($this->getVar('placing_type')) {
            $pt[]=$this->getVar('placing_type');
        }
        if ($this->getVar('placing_type2')) {
            $pt[]=$this->getVar('placing_type2');
        }
        $pt=implode(', ', $pt);
        if ($pt) {
            $pt=' ('.$pt.')';
        ...
        if ($this->getVar('inside_building')) {
            $inside=', Помещение внутри здания ('.$this->getVar('inside_building').')';
        }
    ...
    public function getAllocationBlock()
    {
        ...
        if ($this->getVar('ground_floor')) {
            $out.=$this->getElement('', 'Имеется цокольный этаж');
        }
    ...
    public function getTechnicBlock()
    {
        ...
        if ($this->getVar('electro_reserve')) {
            $electro.=$this->getElement('', 'Есть резервная автономная подстанция');
        }
        if ($this->getVar('electro_not')) {
            $electro.=$this->getElement('', 'Нет электричества');
        }
        if ($this->getVar('electro_allow')) {
            $electro.=$this->getElement('', 'Получена документация для подключения');
        }
        if ($electro) {
        ...
        $infra = '';
        if ($lifts) {
            $infra.=$this->getElement($this->nbsp(4).'Грузоподъемные устройства:', '').$lifts;
        }
    ...
    public function getTransportBlock()
    {
        ...
        if ($this->getVar('realroad_not_active')) {
            $transp.=$this->getElementIf('', 'Не действующие');
        }
        ...
        if ($this->getVar('pandus')) {
            $transp.=$this->getElement('', 'Пандус');
        }
        ...
        if ($this->getVar('parking_underground')) {
            $transp.=$this->getElement($this->nbsp(4).'', 'Подземная');
        }
        if ($this->getVar('parking_many_floor')) {
            $transp.=$this->getElement($this->nbsp(4).'', 'Многоярусная');
        }
        if ($transp) {
        ...
        if ($this->getVar('one_way_traffic')) {
            $road.=$this->getElement($this->nbsp(4).'Одностороннее движение', '');
        }
        if ($road) {
    ...
    public function getFinanceBlock()
    {
        ...
        if ($this->getVar('offer_parts')) {
            $price.= $this->getIf($this->getPriceFromTo($a['price_m2_min'], $a['price_m2_max'], $this->getVar('price_currency')), ' (', '/ '.$price_ground_unit.')');
        }
