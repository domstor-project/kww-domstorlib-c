...
class Domstor_Detail_Supply_Garage extends Domstor_Detail_Supply
{
    ...
    public function getAnnotation()
    {
        ...
        if ($a['size_x'] and $a['size_y']) {
            $annotation.=', '.$a['size_x'].' x '.$a['size_y'].' м';
        }
    ...
    public function getAddress()
    {
        ...
        $space = $out? ', ' : '';
        if ($this->getVar('cooperative_name')) {
            $out.= $space.$this->getVar('cooperative_name');
        }
        return $out;
    }
    ...
    public function getTechnicBlock()
    {
        ...
        if ($this->getVar('build_year')) {
            $state.= $this->getElement($this->nbsp(4).'Год постройки:', $this->getVar('build_year'));
        }
        if ($this->getVar('wearout')) {
            $state.= $this->getElement($this->nbsp(4).'Процент износа: ', $this->getVar('wearout').'%');
        }
        if ($this->getVar('state')) {
            $state.= $this->getElement($this->nbsp(4).'Состояние:', $this->getVar('state'));
        }
        if ($state) {
